fn main() {
    ...
    let overlapping_tasks = str
        ...
        .filter(|pair| {
            ...
            (elf_1_start_num >= elf_2_start_num && elf_1_start_num <= elf_2_end_num)
                || (elf_1_end_num >= elf_2_start_num && elf_1_end_num <= elf_2_end_num)
                || (elf_2_start_num >= elf_1_start_num && elf_2_start_num <= elf_1_end_num)
                || (elf_2_end_num >= elf_1_start_num && elf_2_end_num <= elf_1_end_num)
        })
        .count();

    println!("Overlapping assignment pairs: {:?}", overlapping_tasks);
}
